...
def exitWithError(error):
    print(error)  # Fixed Python 3 print statement
    sys.exit(1)

def runAndReadOutput(args):
    if isinstance(args, str):  # Fixed type check
        args = [args]
    try:
        return subprocess.check_output(args, text=True).strip()  # Decode output
...
def runAndCheckSizes():
    ...
    types = [
        ("int", 4), ("short", 2), ("long", 8), ("char", 1),
        ("float", 4), ("double", 8), ("unsigned int", 4), ("long long", 8),
        ("uint8_t", 1), ("uint16_t", 2), ("uint32_t", 4), ("uint64_t", 8),
        ("uint_fast8_t", 1), ("uint_fast16_t", 8), ("uintmax_t", 8), ("intmax_t", 8),
        ("__int128", 16), ("uint32_t", 4), ("uint64_t", 8), ("student", 8),
        ("x", 20), ("int*", 8), ("short*", 8), ("long*", 8), ("char*", 8),
        ("float*", 8), ("double*", 8), ("unsigned int*", 8), ("long long*", 8),
        ("uint8_t*", 8), ("uint16_t*", 8), ("uint32_t*", 8), ("uint64_t*", 8),
        ("uint_fast8_t*", 8), ("uint_fast16_t*", 8), ("uintmax_t*", 8), ("intmax_t*", 8),
        ("__int128*", 8), ("uint32_t*", 8), ("uint64_t*", 8), ("student*", 8),
        ("&x", 8)
    ]
...
def runAndCheckSwap():
    ...
    if output.strip() != expected_output:
        exitWithError(f'ERROR: actual output: "{output}", expected "{expected_output}"')
